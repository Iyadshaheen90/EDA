...
public class SymbolIO {
    ...
    public SymbolIO(TabPane tabArea, Label mouseCoordinates, ToolBarController toolBar, Accordion sidePanel){
        ...
        symbols.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> {
            if(newValue != null && newValue != oldValue && newValue.isLeaf() && newValue.getValue().endsWith(".eda")){
                ...
                if (Global.retrieveSymbol(fileName) != null){
                    //Do nothing, dont open multiple instances of a symbol
                }
                else{
